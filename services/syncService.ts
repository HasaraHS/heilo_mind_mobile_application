...
export const fetchSyncStatus = async (): Promise<SyncStatus> => {
  try {
    ...
    // Determine last synced time from timestamp field
    const lastSyncedTimestamp = (data.timestamp as number) || Date.now()
    const lastSyncedDate = new Date(lastSyncedTimestamp * 1000) 
